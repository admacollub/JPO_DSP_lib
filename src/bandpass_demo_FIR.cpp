...
int main()
{
    ...
    af::Bandpass<double> BP_filter(44100.0, "HPF", 60 , 500, 2000);
    std::vector<double> coeffs = BP_filter.get_coeff();
    std::cout << "BPF coeffitients: [ ";
    for( double c : coeffs) {
        std::cout << c << " ";
    }
    std::cout << "]" << std::endl;

    std::cout << "Output from BPF [ ";
    for(double s : samples){
        double result = BP_filter.filter(s);
        std::cout << result << " ";
    }
    std::cout << " ]" << std::endl;
