...
int main()
{
    ...
    af::Bandstop<double> BS_filter(44100.0, "HPF", 60 , 500, 2000);
    std::vector<double> coeffs = BS_filter.get_coeff();
    std::cout << "BSF coeffitients: [ ";
    for( double c : coeffs) {
        std::cout << c << " ";
    }
    std::cout << "]" << std::endl;

    std::cout << "Output from BSF [ ";
    for(double s : samples){
        double result = BS_filter.filter(s);
        std::cout << result << " ";
    }
    std::cout << " ]" << std::endl;
